                        ...
                        foreach (var packet in packets)
                        {
                            ...
                            foreach (var packetData in packetDatas)
                            {
                                ...
                                if (packetData.Count == (byteArrayLength))
                                {
                                    ...
                                    #region Force
                                    //convert force
                                    var FORCEMSB = Convert.ToByte(data[12], 16);
                                    var FORCELSB = Convert.ToByte(data[13], 16);
                                    #endregion

                                    #region MSB LSB combination
                                    float elapsedTime = (long)((TIME2MSB & 0xFF) << 24 | (TIME2LSB & 0xFF) << 16 | (TIME1MSB & 0xFF) << 8 | (TIME1LSB & 0xFF));
                                    float force = ((FORCEMSB & 0xFF) << 8 | (FORCELSB & 0xFF));
                                    //float force = 256 * FORCEMSB + FORCELSB;
                                    #endregion
